...
public class FormAgregarProducto_ {
	...
	public FormAgregarVenta getFormag() {
		return formag;
	}

	public void setFormag(FormAgregarVenta formag) {
		this.formag = formag;
	}
	...
	public FormAgregarProducto_(FormAgregarVenta fmag) {
		initialize();
	...
	public void cargarTabla(String val) {
		...
		tablaproducto.setModel(new DefaultTableModel(OpVenta.ListarProducto(val),
		...
		btnagregar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (getIdproducto_() != 0 && btnagregar.isEnabled()) {
					...
					boolean b = true;
					for (int i = 0; i < formag.getDetalleventa().size(); i++) {
						if (formag.getDetalleventa().get(i).getProducto().getIdproducto() == prod.getIdproducto()) {
							b = false;
							i = formag.getDetalleventa().size();
						}
					}
					if (b) {
						DetalleVenta det = new DetalleVenta(prod, 0, prod.getPrecioventa());
						formag.getDetalleventa().add(det);
					}
